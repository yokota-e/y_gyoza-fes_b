...
try {
    ...
    $db = db_connect();
    // プリペアードステートメント作成
    $sql = 'SELECT shops.id AS shop_id,shops.name AS shop_name,menus.name AS menu_name,menus.amount,menus.price,menus.image FROM shops INNER JOIN  menus ON shops.id = menus.id WHERE shops.is_deleted = 0 ORDER BY shops.id ASC';
    $stmt = $db->prepare($sql);
    // SQLの実行
    $stmt->execute();
